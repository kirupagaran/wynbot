    ...
    if intent_name == 'FreeWifi':
        if suburb is None:
            return{
                "dialogAction":{
                    ...
                    "message": {
                        ...
                        "content": "Please specify the Suburb name you are trying to find Free WiFi in Wyndham"
                    },
                    "intentName": "FreeWifi",
                    "slots": slots,
            ...
            if suburb.replace("\"","").replace(" ","").lower() in ["cocoroc","eynesbury","hopperscrossing","laverton","lavertonnorth","littleriver","mambourin","manorlakes","mountcottrell","pointcook","tarneit","quandong","truganina","werribee","werribeesouth","williamslanding","wyndhamvale"]:
                response_data = ""
                with open('/tmp/free_wifi.csv', 'rb') as csvfile:
                    count = 1
                    #suburb = "Little River"
                    for line in csvfile.readlines():
                        ...
                        if(first_item==suburb.replace("\"","").replace(" ","").lower()):
                            response_data += str(count) + ")" + array[1].replace("\"", "")+" [WiFi Name: "+ array[2] + "]" + "   "
                            count+=1
                if response_data == "":
                    response_data = "There are no Free Wifi in "+suburb
                return{
                    "dialogAction": {
                        "type": "Close",
...
s3_client.download_file('wyndhamgovhack', 'free_wifi.csv','/tmp/free_wifi.csv')
#
