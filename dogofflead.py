    ...
    if intent_name == 'DogOfflead':
        if suburb is None:
            ...
            if suburb.replace("\"","").replace(" ","").lower() in ["cocoroc","eynesbury","hopperscrossing","laverton","lavertonnorth","littleriver","mambourin","manorlakes","mountcottrell","pointcook","tarneit","quandong","truganina","werribee","werribeesouth","williamslanding","wyndhamvale"]:
                response_data = ""
                with open('/tmp/dog_off_lead_areas.csv', 'rb') as csvfile:
                    count = 1
                    #suburb = "Little River"
                    for line in csvfile.readlines():
                        ...
                        if(first_item==suburb.replace("\"","").replace(" ","").lower()):
                            response_data += str(count) + ")" + array[1] + "," + array[2].replace("\"", "") + "   "
                            count+=1
                if response_data == "":
                    response_data = "There are no Dog off lead ares in "+suburb
                return{
                    "dialogAction": {
                        "type": "Close",
...
s3_client.download_file('wyndhamgovhack', 'dog_off_lead_areas.csv','/tmp/dog_off_lead_areas.csv')
#
