    ...
    if intent_name == 'SmartBins':
        if suburb is None:
            return{
                "dialogAction":{
                    ...
                    "message": {
                        ...
                        "content": "Please specify the Suburb name you are trying to find Smart Bins in Wyndham"
                    },
                    "intentName": "SmartBins",
                    "slots": slots,
            ...
            if suburb.replace("\"","").replace(" ","").lower() in ["cocoroc","eynesbury","hopperscrossing","laverton","lavertonnorth","littleriver","mambourin","manorlakes","mountcottrell","pointcook","tarneit","quandong","truganina","werribee","werribeesouth","williamslanding","wyndhamvale"]:
                response_data = ""
                with open('/tmp/smartbins.csv', 'rb') as csvfile:
                    count = 1
                    #suburb = "Little River"
                    for line in csvfile.readlines():
                        ...
                        if(first_item==suburb.replace("\"","").replace(" ","").lower()):
                            response_data += str(count) + ")" + array[2].replace("\"", "")+", "+" [TYPE - "+ array[1] + "]" + "   "
                            count+=1
                if response_data == "":
                    response_data = "There are no Smart Bins in "+suburb
                return{
                    "dialogAction": {
                        "type": "Close",
...
s3_client.download_file('wyndhamgovhack', 'smartbins.csv','/tmp/smartbins.csv')
#
