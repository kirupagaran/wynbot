    ...
    if intent_name == 'KinderGartens':
        if suburb is None:
            return{
                "dialogAction":{
                    ...
                    "message": {
                        ...
                        "content": "Please specify the Suburb name you are trying to find Kindergartens in Wyndham"
                    },
                    "intentName": "KinderGartens",
                    "slots": slots,
            ...
            if suburb.replace("\"","").replace(" ","").lower() in ["cocoroc","eynesbury","hopperscrossing","laverton","lavertonnorth","littleriver","mambourin","manorlakes","mountcottrell","pointcook","tarneit","quandong","truganina","werribee","werribeesouth","williamslanding","wyndhamvale"]:
                response_data = ""
                with open('/tmp/kindergartens.csv', 'rb') as csvfile:
                    count = 1
                    #suburb = "Little River"
                    for line in csvfile.readlines():
                        ...
                        if(first_item==suburb.replace("\"","").replace(" ","").lower()):
                            response_data += str(count) + ")" + array[3].replace("\"", "")+", "+array[4].replace("\"", "")+"[operated by "+ array[1] + "]" +"-"+ array[2].replace("\"", "") + "   "
                            count+=1
                if response_data == "":
                    response_data = "There are no Kindergartens in "+suburb
                return{
                    "dialogAction": {
                        "type": "Close",
...
s3_client.download_file('wyndhamgovhack', 'kindergartens.csv','/tmp/kindergartens.csv')
#
